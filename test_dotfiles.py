...
class TestDotfiles:
    ...
    def test_asdf(self, tmux_fake_session: Session) -> None:
        """Verify ASDF version matches the expected version from the Formula (v0.11.2).

        This test ensures that the installed version of ASDF matches what's specified
        in the Formula/asdf@0.11.2.rb file.
        """
        env = shutil.which("env")
        assert env is not None, "Cannot find usable `env` in PATH."

        tmux_fake_session.new_window(attach=True, window_name="test_asdf", window_shell=f"{env} PURE_PROMPT_SYMBOL='>' zsh")
        ...
        # Check ASDF version
        pane.send_keys('asdf --version\n', literal=True, suppress_history=False)
        time.sleep(2)
        pane_contents = "\n".join(pane.capture_pane())

        expected_version = "v0.11.2"
        assert expected_version in pane_contents, f"Expected ASDF version {expected_version} not found in output"

        # Additional check to verify ASDF is properly installed and functioning
        pane.send_keys('asdf plugin list all | head -n 1\n', literal=True, suppress_history=False)
        time.sleep(2)
        pane_contents = "\n".join(pane.capture_pane())
        assert "name" in pane_contents.lower(), "ASDF plugin list command failed to execute properly"
